...
def generate(total,chrom):
    ...
    mix = AudioSegment.from_file(str(chord[0]+keyModifier)+".wav")
    chord.pop(0)
    for note in chord:
        file = AudioSegment.from_file(str(note+keyModifier)+".wav")
        key = AudioSegment.from_file(str(1+keyModifier)+".wav")
